...
use crate::{
    ...
    serialization::coption_fromstr,
    system_instruction::{SYSTEM_PROGRAM_ID, SYSVAR_ID},
    token_error::TokenError,
};
use ic_solana::types::{AccountMeta, Instruction, Pubkey, PUBKEY_BYTES};
...
pub const TOKEN22_PROGRAM_ID: &str = "TokenzQdBNbLqP5VEhdkAS6EPFLC1PHnBqCXEpPxuEb";
...
pub fn initialize_mint(
    ...
    let accounts = vec![
        ...
        AccountMeta::new_readonly(Pubkey::from_str(SYSVAR_ID).unwrap(), false),
    ];
...
mod test {
    ...
    fn test_initialize_mint_packing() {
        let decimals = 2;
        let mint_authority = ic_solana::types::Pubkey::new([1u8; 32]);
        ...
        assert_eq!(pod.mint_authority.to_bytes(), mint_authority.to_bytes());
        assert_eq!(pod_freeze_authority.is_none(), freeze_authority.is_none());

        let mint_authority = ic_solana::types::Pubkey::new([2u8; 32]);
        let freeze_authority = COption::Some(ic_solana::types::Pubkey::new([3u8; 32]));
        let check = TokenInstruction::InitializeMint {
            decimals,
            mint_authority,
            freeze_authority,
    ...
    fn test_set_authority_packing() {
        ...
        let new_authority = COption::Some(ic_solana::types::Pubkey::new([4u8; 32]));
        ...
        // assert_eq!(pod_new_authority, new_authority.unwrap());
    }
    ...
    fn test_initialize_account2_packing() {
        let owner = ic_solana::types::Pubkey::new([2u8; 32]);
        ...
        assert_eq!(pod_owner.to_bytes(), owner.to_bytes());
    }
    ...
    fn test_initialize_mint_close_authority_packing() {
        let close_authority = COption::Some(ic_solana::types::Pubkey::new([10u8; 32]));
    ...
    fn test_initialize_permanent_delegate_packing() {
        let delegate = ic_solana::types::Pubkey::new([11u8; 32]);
        ...
        assert_eq!(pod_delegate.to_bytes(), delegate.to_bytes());
    }
}
