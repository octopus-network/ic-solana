...
use ic_solana::types::UiTransactionEncoding;
use ic_solana::types::{
    Account, BlockHash, EncodedConfirmedTransactionWithStatusMeta, EncodingConfig, Instruction,
    Message, Pubkey, RpcAccountInfoConfig, RpcContextConfig, RpcSendTransactionConfig,
    RpcSignatureStatusConfig, RpcTransactionConfig, Signature, Transaction, TransactionStatus,
    UiAccountEncoding, UiTokenAmount,
};
...
pub async fn sol_get_transaction(signature: String) -> RpcResult<String> {
    ...
    let response = client
        ...
        .await?;
    ic_cdk::println!("transaction detail: {:#?}", response);
    Ok(response)
}
